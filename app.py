...
def tem_conflito(data, horario_novo, servico_novo):
    ...
    fim_novo = inicio_novo + timedelta(minutes=minutos_servico(servico_novo))
    ...
    for horario_existente, servico_existente in agendamentos:
        inicio_existente = horario_para_datetime(data, horario_existente)
        fim_existente = inicio_existente + timedelta(minutes=minutos_servico(servico_existente))
...
def horarios_ocupados():
    ...
    for horario in horarios_base:
        if tem_conflito(data, horario, "Corte"):
...
def agendar():
    ...
    if tem_conflito(data, horario, servico):
        return "Esse horário entra em conflito com outro agendamento. Volte e escolha outro horário."
    ...
    cursor.execute("""
        ...
        (nome, telefone, servico, data, horario, observacao)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (nome, telefone, servico, data, horario, observacao))
